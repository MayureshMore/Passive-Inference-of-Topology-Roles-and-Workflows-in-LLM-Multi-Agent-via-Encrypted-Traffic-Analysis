...
#
# ADDITIVE: writes only its OUT dir. Local Ollama, $0.
# Usage: bash scripts/collect_wf_interleaved.sh [CYCLES] [N_PER_BATCH] [TOPOS] [OUT]
#   TOPOS default "star" (the original control); pass "star chain mesh" for the full-topology
#   version. Workflows are round-robined (temporally interleaved) within each topology block.
set -u
CYCLES="${1:-6}"; NB="${2:-6}"; TOPOS="${3:-star}"
PY="venv/bin/python"; OUT="${4:-data/raw_wf_interleaved}"; URL="http://localhost:11434"
...
for top in $TOPOS; do
  ...
  for c in $(seq 1 "$CYCLES"); do
    ...
    echo "=== $top cycle $c/$CYCLES  seed_offset=$off ==="
    for k in 0 1 2 3; do
        wf=${WF[$(( (k + r) % 4 ))]}
        $PY scripts/run_pilot.py --deployment a --workflow "$wf" --topology "$top" \
...
echo "DONE wf-interleaved ($TOPOS): $(count) pcaps  ->  $OUT"
