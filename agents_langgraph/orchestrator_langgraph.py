...
The independent execution model: call count, ordering, and parallel fan-out are
produced by a **LangGraph `StateGraph`** (Pregel supersteps + a reducer channel),
NOT by hand-rolled `asyncio.gather` (deployment A) or a hand-rolled sequential
loop (deployment B).  Everything that defines the *labels* is held identical to A:
...
Only the *engine that schedules the delegation* differs.  The A→C transfer
experiment then tests whether the traffic fingerprint survives an independent
orchestration framework (the cross-framework counterpart to the hand-built
deployment B).
