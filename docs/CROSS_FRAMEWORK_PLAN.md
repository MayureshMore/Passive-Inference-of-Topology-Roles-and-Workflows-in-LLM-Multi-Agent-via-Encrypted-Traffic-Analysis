...
3. **Interpretation guide (decide which story the data tells):**
   ...
   |---|---|---|
   | within-A high, within-C high, **A→C low** | Non-transfer holds across an independent framework | **Main hoped result** — strengthens the central claim. Report directly. |
   | within-A high, within-C high, **A→C high** | Fingerprint generalizes across frameworks | **Reframe**, don't bury: the claim becomes "transfer tracks structural similarity," which is *more* interesting. Flag for the writeup. |
   | within-C low | C produces no learnable fingerprint | **Gate failure** — debug C (Phase 1.5/2), do not interpret transfer. |
...
1. **§5.3** (cross-implementation): add a cross-framework table (within-A,
   ...
   across *both* a hand-built variant (B) *and* an independent framework (C).
...
4. **New figure**: cross-framework transfer matrix (or extend the cross-deployment
   figure). Add to `scripts/make_paper_artifacts.py` reading
